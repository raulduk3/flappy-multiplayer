    ...
    try {
      ...
      await new Promise<void>((resolve, reject) => {
        const timer = setTimeout(
          () => reject(new Error("timeout waiting for visibility")),
          10000,
        );
        ...
        let player1RunId: string | null = null;
        let player2RunId: string | null = null;
        ...
        ws1.on("message", (data) => {
          ...
          messages1.push(msg);
          if (msg.type === "join.ack") s1Joined = true;
          if (msg.type === "runStart.event") player1RunId = msg.payload.run_id;
          ...
          if (msg.type === "runEnd.event") {
            if (!bothSeenSimul) {
              ...
              reject(
                new Error(
                  "runEnd occurred before both players were visible simultaneously",
                ),
              );
              return;
            }
              ...
              try {
                ...
                for (const s of [...laterSnaps1, ...laterSnaps2]) {
                  ...
                  if (players && players.length > 0) {
                    expect(
                      players.find((p: any) => p.run_id === runEndId),
                    ).toBeUndefined();
                  }
                }
                clearTimeout(timer);
        ...
        ws2.on("message", (data) => {
          const msg = JSON.parse(data.toString());
          messages2.push(msg);
          if (msg.type === "join.ack") s2Joined = true;
          if (msg.type === "runStart.event") player2RunId = msg.payload.run_id;
          if (msg.type === "snapshot.event") {
            const players = msg.payload.players as any[];
            if (player1RunId && player2RunId) {
              const ids = new Set(players.map((p) => p.run_id));
