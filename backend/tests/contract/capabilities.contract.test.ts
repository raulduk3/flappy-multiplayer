...
describe('Capabilities Contract (T007)', () => {
  ...
  const reqPath = path.join(__dirname, '../../../shared/schemas/protocol/v1/capabilities-request.schema.json');
  const resPath = path.join(__dirname, '../../../shared/schemas/protocol/v1/capabilities-response.schema.json');

  let reqSchema: any; let resSchema: any;
  ...
  test('response schema required fields and array shape', () => {
    ...
    const good = {type: 'capabilities_response', protocol_version: '1.0.0', supported_features: ['snapshot']};
    expect(validate(good)).toBe(true);

    const emptyFeatures = {type: 'capabilities_response', protocol_version: '1.0.0', supported_features: []};
