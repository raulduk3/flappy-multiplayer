...
describe("Entity Schemas Contract (T008)", () => {
  ...
  function loadSchema(name: string) {
    ...
    if (!fs.existsSync(p))
      throw new Error(`Missing schema: ${p} (implement T017-T019)`);
  ...
  test("LeaderboardEntry ranking tie-break docs present", () => {
    ...
    expect(schema.required).toEqual(
      expect.arrayContaining(["player_id", "name", "score", "rank", "at"]),
    );
  });
  ...
  test("Capabilities request/response minimal shape", () => {
    ...
    expect(
      vReq({ type: "capabilities_request", protocol_version: "1.0.0" }),
    ).toBe(true);
    expect(
      vRes({
